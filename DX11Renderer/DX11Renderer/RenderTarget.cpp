...
namespace Bind
{
	...
	void RenderTarget::BindAsBuffer(Graphics& gfx, BufferResource* depthStencil)
	{
		assert(dynamic_cast<DepthStencil*>(depthStencil) != nullptr);
		BindAsBuffer(gfx, static_cast<DepthStencil*>(depthStencil));
	}

	void RenderTarget::BindAsBuffer(Graphics& gfx, DepthStencil* depthStencil)
	{
	...
	void RenderTarget::Clear(Graphics& gfx, const std::array<float, 4>& color)
	{
		SETUP_LOGGING_NOHR(gfx);
