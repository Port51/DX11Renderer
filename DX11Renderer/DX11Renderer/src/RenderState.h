...
namespace gfx
{
	...
	public:
		...
		const bool IsNewBinding(const guid64 guid, const RenderBindingType bindingType, const slotUINT slot);
		const bool HasBinding(const RenderBindingType bindingType, const slotUINT slot) const;
		void ClearBinding(const RenderBindingType bindingType, const slotUINT slot);
	private:
		const u16 GetKey(const RenderBindingType bindingType, const slotUINT slot) const;
	private:
