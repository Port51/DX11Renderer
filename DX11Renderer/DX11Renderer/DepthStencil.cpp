...
namespace Bind
{
	...
	void DepthStencil::BindAsBuffer(Graphics& gfx, BufferResource* renderTarget)
	{
		assert(dynamic_cast<RenderTarget*>(renderTarget) != nullptr);
		BindAsBuffer(gfx, static_cast<RenderTarget*>(renderTarget));
	}

	void DepthStencil::BindAsBuffer(Graphics& gfx, RenderTarget* rt)
	{
	...
	void ShaderInputDepthStencil::Bind(Graphics& gfx, UINT slot)
	{
		SETUP_LOGGING_NOHR(gfx);
	...
	void OutputOnlyDepthStencil::Bind(Graphics& gfx, UINT slot)
	{
